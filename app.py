...
actions = ['cafe', 'promise', 'name','you','itda','upda','good','real','wait','keep'
           ,'me','glasses','house','mountain','pretty']
seq_length = 30
        ...
        if result.multi_hand_landmarks is not None:
            for res in result.multi_hand_landmarks:
                ...
                if all(action == action_seq[-1] for action in action_seq[-7:]):
                    ...
                    if last_action != this_action:                         ##########여기추가
                        if this_action == 'cafe':
                            text = '카페'
                        elif this_action == 'promise':
                            text = '약속'
                        elif this_action == 'name':
                            text = '이름'
                        elif this_action == 'you':
                            text = '너'
                        elif this_action == 'itda':
                            text = '있다' 
                        elif this_action == 'upda':
                            text = '없다'
                        elif this_action == 'good':
                            text = '좋습니다'
                        elif this_action == 'real':
                            text = '진짜'
                        elif this_action == 'wait':
                            text = '기다리다'
                        elif this_action == 'keep':
                            text = '지키다'
                        elif this_action == 'me':
                            text = '나'
                        elif this_action == 'glasses':
                            text = '안경'
                        elif this_action == 'house':
                            text = '집'
                        elif this_action == 'mountain':
                            text = '산'
                        elif this_action == 'pretty':
                            text = '이쁘다'
